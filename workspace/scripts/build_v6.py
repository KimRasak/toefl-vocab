...
用法：
  python3 workspace/scripts/build_v6.py
...
def main():
    cam_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(GENERATED_DIR, "cambridge_defs_1791.json")
    src_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(ROOT, "site", "vocab", "1791", "index.html")
    out_path = sys.argv[3] if len(sys.argv) > 3 else os.path.join(ROOT, "site", "vocab", "1791", "index.html")
    txt_path = sys.argv[4] if len(sys.argv) > 4 else os.path.join(VOCAB_DIR, "my-tofel-1791words-words.txt")

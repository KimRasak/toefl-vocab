...
- 输出：site/vocab/1791/audio/ + audio_map.js（词 -> {w, e} 本地路径）
...
SITE_DIR = os.path.join(BASE, "site", "vocab", "1791")
AUDIO_DIR = os.path.join(SITE_DIR, "audio")
OUT_MAP = os.path.join(SITE_DIR, "audio_map.js")
HTML_PATH = os.path.join(SITE_DIR, "index.html")
DATA_PATH = os.path.join(WORKSPACE, "_local", "generated", "output", "cambridge_defs_1791.json")
...
async def main():
    ...
    mapping = {}
    for i, (w, _zh) in enumerate(words):
        wpath = f"audio/{i:04d}-w-{slugify(w)}.mp3"
        epath = f"audio/{i:04d}-e-{slugify(w)}.mp3"
        entry = {}

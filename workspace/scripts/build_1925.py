...
def main():
    cam_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(GENERATED_DIR, "cambridge_defs_1925.json")
    txt_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(VOCAB_DIR, "my-tofel-1925words-words.txt")
    out_path = sys.argv[3] if len(sys.argv) > 3 else os.path.join(ROOT, "site", "vocab", "1925", "index.html")
    tpl_path = sys.argv[4] if len(sys.argv) > 4 else os.path.join(ROOT, "site", "vocab", "1791", "index.html")
    ...
    # 5) 导航链接
    tpl = tpl.replace('<a class="link-btn" href="../1675/">→ 1675</a>',
                      '<a class="link-btn" href="../1791/">→ 1791</a>', 1)

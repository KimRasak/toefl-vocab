...
Usage:  python3 workspace/scripts/build_merged_by_discipline.py
Output: site/vocab/disciplines/index.html
"""
import os, re, json
...
VOCAB_DIR = os.path.join(WORKSPACE, 'data', 'vocabulary')
ANALYSIS_DIR = os.path.join(WORKSPACE, 'data', 'analysis')
OVERRIDES_DIR = os.path.join(WORKSPACE, 'data', 'overrides')
GENERATED_DIR = os.path.join(WORKSPACE, '_local', 'generated', 'output')
...
DEFINITION_OVERRIDES_PATH = os.path.join(OVERRIDES_DIR, 'overrides.json')
try:
...
try:
    with open(os.path.join(OVERRIDES_DIR, 'pos_overrides.json'), encoding='utf-8') as _f:
        POS_OVERRIDES = {canonical_word(k): str(v).strip() for k, v in json.load(_f).items()}
except (FileNotFoundError, json.JSONDecodeError):
    POS_OVERRIDES = {}
try:
    with open(os.path.join(OVERRIDES_DIR, 'pos_review_candidates.json'), encoding='utf-8') as _f:
...
try:
    with open(os.path.join(OVERRIDES_DIR, 'definition_corrections.json'), encoding='utf-8') as _f:
...
HIGH_FREQ_DISCIPLINE_PATH = os.path.join(ANALYSIS_DIR, 'high_frequency_discipline_words.json')
try:
...
EXAM2026_PATH = os.path.join(ANALYSIS_DIR, 'toefl2026_exam_terms.json')
...
def parse_1791():
    ...
    path = os.path.join(VOCAB_DIR, 'my-tofel-1791words-words.txt')
    if not os.path.exists(path):
        path = os.path.join(VOCAB_DIR, 'tofel_words_1791.txt')
...
def parse_1925():
    ...
    words = {}
    path = os.path.join(VOCAB_DIR, 'my-tofel-1925words-words.txt')
    with open(path, encoding='utf-8') as f:
        for line in f:
            w, d = parse_word(line)
            if w:
...
def parse_1675():
    ...
    words = {}
    topics = {}
    path = os.path.join(VOCAB_DIR, 'my-tofel-1675words-words.txt')
    with open(path, encoding='utf-8') as f:
        lines = f.readlines()
...
def main():
    ...
    scenario_path = os.path.join(ANALYSIS_DIR, 'new_scenario_words.json')
    scenario_topics = {}
    scenario_words = {}
    try:
    ...
    print("Done! Output: site/vocab/disciplines/index.html")
...
def generate_html(grouped, total):
    out_dir = os.path.join(BASE, 'site', 'vocab', 'disciplines')
...
<header>
  <div class="top-row">
    <a class="home-btn" href="../../index.html">← 首页</a>

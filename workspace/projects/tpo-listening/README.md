...
```
workspace/projects/tpo-listening/
├── README.md            # 本说明
...
├── build_gh_full_player.py  # 构建 site/listening/all/index.html
├── test_player.js       # 全库播放器的无头回归测试（node test_player.js）
└── test_player.js       # 直接验证 site/listening/all/index.html
```

## 全库播放器的构建与验证

```bash
python3 workspace/projects/tpo-listening/build_gh_full_player.py
node test_player.js                 # 34 条断言，退出码非 0 即失败
```

播放器功能：按套号/年代/学科领域排序，按类型、学科领域、进度（未听/已听）筛选，
搜索覆盖套号、标题、学科名与年份，进度面板按领域给出已听数与剩余时长，
另有一键把 200 集精选页（`site/listening/selected/`）的「听完」标记搬过来的按钮

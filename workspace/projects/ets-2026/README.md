...
- `extract_audio.py`：把 JSON 引用的 196 个音频从 ZIP 解出并摊平到 `site/listening/2026/audio/`
- `audio_len.py`：不依赖 ffmpeg 读音频时长（ogg 走 granulepos 精确值，mp3 按比特率估算）
- `build_2026_player.py`：生成 `site/listening/2026/index.html` 练习页
...
python3 extract_audio.py         # zip -> site/listening/2026/audio/ + audio_map.json
python3 build_2026_player.py     # -> site/listening/2026/index.html
node test_2026_player.js         # 48 条断言，退出码非 0 即失败
```

页面在 `site/listening/2026/`：按四类题型分 tab，可按卷 / Module / 进度筛选，

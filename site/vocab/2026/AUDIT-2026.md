...
- **选分区**：下拉列出全部 192 个分区（`c` 值），按 8 个宏类 optgroup 分组、组内保持词库顺序，每项带词数；上次选择存 `toefl2026-settings`，子路径页词库不同时自动回退到第一个分区。分类一览每个子场景标题右侧新增「▶ 连读」按钮，点击即切到连读页从头播放该分区。
...
**回归测试 188 → 220 条全绿**（新增 32 条连读断言：状态初始化、分区分组=唯一分区数、词表渲染、开始即发起 TTS 请求、gap 定时器推进、⏮不越界、播完停止/循环两种收尾、暂停继续、其他入口发音互斥、词表行跳读、一览「▶ 连读」入口、浮动胶囊显隐、8 子页含面板、连读不改写进度）。`Audio` 桩升级为记录 src 的 getter/setter（`audioLog`）以断言发音请求。另把两条按旧词量快照的阈值断言随 e098230 去重后的实际数据放宽：作业任务 ≥25（现 29）、时间日期 ≥25（现 38）。

...
新增学习体验功能：**本轮答错的词立即重练**（直接服务于「我可能不会的词」）。

- `studyResults.wrongList` 收集本轮所有答错词（不重复）
- 学习小结页新增「🔁 重练错词 (N)」按钮（有答错时才出现），并提示已进入记忆曲线回炉队列
- `startErrorRedo()` 用答错词建队列立即重练；与「今日复习」的记忆曲线队列互补（即时、聚焦 vs 定时复习）

**数据零改动**——仅页面功能 + studyResults 结构。回归测试 164 → 167 条全绿（新增：答错词收集、小结按钮、重练队列仅含答错词）。

## 第二十五轮：点评词表发现 8 个「被埋没的词」提级（3367 不变）

应要求点评当前词表。整体质量高：3367 条 / 191 分类，无结构缺失（POS/释义零缺失）、优先级合法、零同分类重复（39 组跨分类重复均为场景差异化、各有独立进度槽）、r=5 重点词校准到位（习语/连接/模糊表达/学术搭配）。

**发现一个违背核心需求的缺陷**：8 个 CET6-555 大概率不会的词被标成 r=1「已知」（权重 0.2），沉在学习队列最底，几乎永不出现在「学新词」中。已全部提级为 r=3（与普通未知词同权，正常入队）：
...
## 第二十七轮：所有技能卡「一览全部」→ 独立子路径页（8 个子路径）

应要求：主站总览每张技能卡点「一览全部 N 词」不再页内切换，而是**跳转到各自的子路径 URL**。

- **8 个子路径页**（每页 = 该宏过滤后的词库 + 主站页面副本，全部带主站全量索引 `i`，进度/同步码与主站完全互通，逻辑同第二十六轮听力版）：
  - `awl/` AWL学术词 574 · `listening/` 听力场景 2042 · `reading/` 日常阅读 79 · `subject/` 学科主题 294
  - `phrasal/` 短语动词 87 · `writing/` 写作表达 197 · `speaking/` 口语表达 18 · `other/` 其他 76
- **主站**新增 `MACRO_SUBPATH` 映射，「一览全部」→ `location.href = 子路径`；**子路径页自身**把该常量置空，一览全部仍走页内分类一览（避免跳到自己嵌套）。
- 生成器 `gen_macro_pages.js`（幂等，可重复运行）：主站 data.js/index.html 零改动，8 页一键重建，后续数据更新后重跑即可。
- 回归测试 **176 → 181 条全绿**（新增：8 页词数=主站对应宏、索引一一对应、补丁齐全、合计 3367、主站映射完整、跳转逻辑、子路径页页内浏览）。

## 第二十八轮：子路径页一打开直接落在该部分单词表（修复交互）

用户反馈：点「一览全部」跳转到子路径页后，看到的是**总览 dashboard** 而不是单词表，需再点一次才看到词，交互不对。

**修复**：生成器 `gen_macro_pages.js` 新增替换——子路径页把 init 的初始 tab 逻辑改为强制 `currentTab='browse'` + `browseMacro=<该宏>`，即**打开页面直接显示该部分的分类单词表**（按子场景分组的词+释义列表），不再经过总览；8 页全部重建。
...
## 第三十轮：分区连读——选一个分区自动朗读，间隔与播完后行为可选

需求来自用户：给主站 `toefl-2026-vocab/` 加「连读」，每次选中一个分区播放；播放间隔可选；播完后可选停止或循环播放。

**实现**（新增「🎧 连读」底栏标签页）：

- **选范围（只整类，不选子场景）**：下拉只列 8 个宏类（AWL学术词 / 听力场景 / 日常阅读 / 学科主题 / 短语动词 / 写作表达 / 口语表达 / 其他），选「听力场景」即朗读该类全部词条，不再出现「听力-餐饮」这类细分。旧设置若存了子场景名，自动回退到所属整类。入口两处：总览类别卡片「🎧 连读」、连读页下拉。分类一览子场景标题上的「▶ 连读」已去掉。
- **间隔可选**：无间隔 / 0.5 / 1 / 2 / 3 / 5 秒（默认 1 秒）。间隔是「上一词播完 → 下一词开声」的停顿；等待期间改动下拉立即按新值重排。
- **播完后可选**：停止（回第 1 词待开始，toast 提示读完）/ 循环播放（回第 1 词继续）。
- **控制**：开始/暂停/继续共用主按钮；⏮/⏭ 步进（连读中点击分区词表任意词=跳到该词继续）；「跳到第 N 词」输入框；可勾选「只连读收藏的词」。⏹ 停止按钮只在播放/暂停时出现在开始按钮右侧（窄图标），不再挤在 ⏮/⏭ 中间。分区词表实时高亮正在朗读的词并滚动跟随。页面底部预留底栏高度，瀑布流「显示更多」不被挡住。
- **跨标签页**：连读中切到其他标签页不中断，底部浮动胶囊显示「🎧 当前词 x/N」，点击回连读页、⏹ 一键停止。任何其他入口发音（卡片🔊、词表行、会话自动开读）先自动停连读，避免两路声音打架。
- **链路复用**：每词仍走 `speak()` 全链路（本地 TTS → 有道/谷歌/百度回退 + `ac` 分块连播 + 句型槽位朗读），播完由 `onDone` 驱动 `seqAdvance → 间隔 → seqStep(1)`；`seqToken` 在每次打断/步进自增，作废过期的播完回调，防止双推进。连读不写 `progress`，标记仍走卡片流程。`pagehide` 时整体停止。

**子路径页**：连读面板随 `gen_macro_pages.js` 重建自动带到 8 个子路径页（每页下拉只列本宏整类一项）。本次重建顺带修复子路径页自 29 轮后的过期：listening 子页 data.js 仍是去重前的 2136 词（主库 2098），且各子页缺「会/不会」学习模式等已提交改动；重建后 8 页合计 3423，与主库一致。

**回归测试 225 条全绿**（连读断言覆盖整类下拉 8 项且不含子场景、整类词量/显示名、词表渲染、TTS、间隔推进、⏮不越界、播完停止/循环、暂停继续、其他入口发音互斥、词表行跳读、总览卡片入口、一览无子场景连读按钮、浮动胶囊显隐、8 子页含面板、连读不改写进度）。另把两条按旧词量快照的阈值断言随 e098230 去重后的实际数据放宽：作业任务 ≥25（现 29）、时间日期 ≥25（现 38）。

## 第三十一轮：entrée/açai 有道 US 音源坏档修复 + 子路径页 at/ac 字段丢失

用户反馈：`entrée` 卡片音频读音不对。逐源核查后确认这**不是本地 TTS 的问题，而是有道词典音源坏档**，并连带发现一个子路径页字段丢失的潜伏 bug。

### 根因：有道 dictvoice 的「音符词」US 音档是逐字母读音

单词卡远程链路第一候选是有道 US（`dictvoice?audio=<词>&type=2`）。用 whisper（small/medium/large-v3-turbo 三档模型）对照剑桥词典官方录音做转写核验：

- `audio=entrée&type=2` → 返回 160kbps 音档，读的是「Enter E」（三个模型一致），4 个能量峰；剑桥 ground truth /ˈɒntreɪ/ 只有 2 个峰。
- `audio=entree&type=2`（去音符拼写）→ 读出规范 ˈɒntreɪ；MFCC-DTW 距离 0.965（与剑桥 US 录音同源级别，对照 youdao UK 为 1.428）。
- 同病：`açai`（听出发音陷阱卡）`type=2` 返回 160kbps 档读「A.A.I.」，完整拼写 `açaí` 读「Uh-uh」；只有无音符 `acai` 正常。
- 坏档全部集中在 160kbps 这一档；`café`(768kbps)、`fiancé`/`résumé`/`cliché`(正常 64-67kbps 档) 转写均与剑桥一致，不动。

坏档会正常出声（HTTP 200、能播放），所以候选链不会降级——用户听到的就是它。本地 TTS（macOS Samantha 等对「entrée」读音正确）只在 speechSynthesis 可用时先行；一旦设备无英文声音/超时/关了发音开关，就落到坏档。

### 修复：词条级 `at` 音频文本覆盖（沿用既有机制）

`data.js` 给两条词条加 `at`（显示词不变，仅发音请求文本换拼写）：

- `entrée` → `"at":"entree"`；`açai` → `"at":"acai"`。

全链路复核过新请求文本：有道 US/UK、百度、谷歌、本地 Samantha 对「entree」全部读出正确发音（谷歌读「Entrez」、百度读「Entree」）；「acai」有道 192kbps 档与剑桥同为 4 峰。

### 潜伏 bug：gen_macro_pages.js 丢字段

`ser()` 固定序列化 `w,p,m,c,i,r` 六字段，主库里已有的 **6 条短语 `at` 覆盖与 13 条 `ac` 分块**在 8 个子路径页上被静默丢弃（「With all due respect」等的大小写覆盖在子页全部失效）。改为可选字段条件序列化后重建 8 页，主库/子页字段逐字一致；index.html 重建零差异（幂等）。

### 测试

新增 5 条断言：entrée/açai 词条带 `at`、`speak('entrée')`/`speak('açai')` 发出的首个远程请求 URL 正是无音符拼写（audioLog 直查）、`at` 不改显示词；子页一致性检查扩到 `at`/`ac` 逐字随行（丢字段即 FAIL）。**277/277 全绿**。

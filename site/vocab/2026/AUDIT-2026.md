...
- **选范围（子场景或整类）**：下拉按 8 个宏类 optgroup 分组，每组第一项是**整类连读**（值为宏 key，如「听力场景」全部 2098 词），其后是 192 个子场景（`c` 值，保持词库顺序，带词数）；上次选择存 `toefl2026-settings`，子路径页词库不同时自动回退到第一项。入口共三处：总览类别卡片「🎧 连读」（=整类）、分类一览子场景标题右侧「▶ 连读」（=该子场景）、连读页下拉自由切换。用户澄清「分区」需支持选中一整个类别（如听力场景）后补齐整类选项。
- **间隔可选**：无间隔 / 0.5 / 1 / 2 / 3 / 5 秒（默认 1 秒）。间隔是「上一词播完 → 下一词开声」的停顿；等待期间改动下拉立即按新值重排。
- **播完后可选**：停止（回第 1 词待开始，toast 提示读完）/ 循环播放（回第 1 词继续）。
- **控制**：开始/暂停/继续共用主按钮；⏮/⏭ 步进（连读中点击分区词表任意词=跳到该词继续）；⏹ 停止并复位。分区词表实时高亮正在朗读的词并滚动跟随。
...
**子路径页**：连读面板随 `gen_macro_pages.js` 重建自动带到 8 个子路径页（各自只列本宏分区，awl:10 / listening:130 / reading:9 / subject:18 / phrasal:1 / writing:21 / speaking:1 / other:2 个分区组）。本次重建顺带修复子路径页自 29 轮后的过期：listening 子页 data.js 仍是去重前的 2136 词（主库 2098），且各子页缺「会/不会」学习模式等已提交改动；重建后 8 页合计 3423，与主库一致。

**回归测试 188 → 226 条全绿**（新增 38 条连读断言：状态初始化、分区分组=唯一分区数、整类选项与整类词量/显示名、词表渲染、开始即发起 TTS 请求、gap 定时器推进、⏮不越界、播完停止/循环两种收尾、暂停继续、其他入口发音互斥、词表行跳读、一览与总览卡片连读入口、浮动胶囊显隐、8 子页含面板与整类选项、连读不改写进度）。`Audio` 桩升级为记录 src 的 getter/setter（`audioLog`）以断言发音请求。另把两条按旧词量快照的阈值断言随 e098230 去重后的实际数据放宽：作业任务 ≥25（现 29）、时间日期 ≥25（现 38）。
